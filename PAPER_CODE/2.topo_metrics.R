...
#### DATA ####
source("rFunctions/modularity_beckett.R")
...
# OUT DEGREE - GENERALITY
generality <- function(tibble){
  prey <- vector()
  for (i in 1:nrow(tibble)){
    k <- 0
...
# IN DEGREE - VULNERABILITY #
vulnerability <- function(tibble){
  pred <- vector()
  for (j in 2:ncol(tibble)){
    k <- 0
    for (i in 1:nrow(tibble)){
...
topo_list <- mclapply(list_mat_int[1:562], network_topo, mc.cores = 52)
...
#write.csv(topo_df, "PAPER_DATA/script_output/topology.csv", row.names = F)
