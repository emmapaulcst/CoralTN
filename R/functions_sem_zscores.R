...
gives_zdata_SEM <- function(bga, flux_per_prey_Ic, z_scores){
  ...
  data <- left_join(bga, flux, by = c("SiteCode" = "site")) %>% 
    select(SiteCode, Realm,
           S, L, C, Cc, nCc, Bc, nBc, G, V, NODF2, Qn, zN, zQn, 
...
make_zSEM <- function(data_sem){
  ...
  bAut <- bf(bAut ~ 1 + algae + turf + gravity + S + C + zN + zQn + (1|Realm), family = Beta)
...
# save(list = "fit_sem", file = 'PAPER_DATA/z_scores/fit_z_sem.rdata')

make_zSEM_5000 <- function(data_sem){
  #### SEM ####
